...
public interface ReservaServicioRepository extends JpaRepository<ReservaServicio, ReservaServicioPK> {
    ...
    @Query(value = "SELECT * FROM reservas_servicio WHERE id_habitacion = :idHabitacion AND id_servicio = :idServicio "
            + "AND fecha = :fecha AND hora_inicio = :horaInicio AND hora_fin = :horaFin AND costo = :costo", nativeQuery = true)
    ReservaServicio buscarReservaServicioPorPK(@Param("idHabitacion") int idHabitacion,
            @Param("idServicio") int idServicio, @Param("fecha") Date fecha, @Param("horaInicio") Time horaInicio,
            @Param("horaFin") Time horaFin, @Param("costo") int costo);

    // Método para insertar una nueva reserva de servicio
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO reservas_servicio (id_habitacion, id_servicio, fecha, hora_inicio, hora_fin, costo) "
            + "VALUES (:idHabitacion, :idServicio, :fecha, :horaInicio, :horaFin, :costo)", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE reservas_servicio SET fecha = :fecha, hora_inicio = :horaInicio, hora_fin = :horaFin, "
            + "costo = :costo WHERE id_habitacion = :idHabitacion AND id_servicio = :idServicio", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM reservas_servicio WHERE id_habitacion = :idHabitacion AND id_servicio = :idServicio "
            + "AND fecha = :fecha AND hora_inicio = :horaInicio AND hora_fin = :horaFin AND costo = :costo", nativeQuery = true)
