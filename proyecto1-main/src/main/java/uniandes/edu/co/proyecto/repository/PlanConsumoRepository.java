...
public interface PlanConsumoRepository extends JpaRepository<PlanConsumo, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO planes_consumo (tipo_plan, tasa, min_noches, max_noches) VALUES (:tipoPlan, :tasa, :minNoches, :maxNoches)", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE planes_consumo SET tipo_plan = :tipoPlan, tasa = :tasa, min_noches = :minNoches, max_noches = :maxNoches WHERE id = :id", nativeQuery = true)
