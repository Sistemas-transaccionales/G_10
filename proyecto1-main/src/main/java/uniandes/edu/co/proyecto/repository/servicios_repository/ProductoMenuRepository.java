...
public interface ProductoMenuRepository extends JpaRepository<ProductoMenu, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO productos_menu (id, nombre, costo) VALUES (:id, :nombre, :costo)", nativeQuery = true)
    void insertarProductoMenu(@Param("id") int id, @Param("nombre") String nombre, @Param("costo") String costo);
