...
public class TipoBebidaController {
    ...
    public String mostrarResultados(Model model) {
        ...
        List<TipoBebida> tiposBebida = mongoTemplate.aggregate(aggregation, "tipo_bebidas", TipoBebida.class)
                .getMappedResults();
    ...
    /**
     * @PostMapping
     *              public ResponseEntity<TipoBebida> crearTipoBebida(@RequestBody
     *              TipoBebida tipoBebida) {
     *              TipoBebida nuevoTipoBebida =
     *              tipoBebidaRepository.save(tipoBebida);
     *              return new ResponseEntity<>(nuevoTipoBebida,
     *              HttpStatus.CREATED);
     *              }
     * 
     * @GetMapping
     *             public ResponseEntity<List<TipoBebida>>
     *             obtenerTodasLasTipoBebidas() {
     *             List<TipoBebida> tipoBebidas = tipoBebidaRepository.findAll();
     *             return new ResponseEntity<>(tipoBebidas, HttpStatus.OK);
     *             }
     * 
     *             @GetMapping("/{id}")
     *             public ResponseEntity<TipoBebida>
     *             obtenerTipoBebidaPorId(@PathVariable String id) {
     *             Optional<TipoBebida> tipoBebida =
     *             tipoBebidaRepository.findById(id);
     ...
     *             @PutMapping("/{id}")
     *             public ResponseEntity<TipoBebida>
     *             actualizarTipoBebida(@PathVariable String id, @RequestBody
     *             TipoBebida tipoBebida) {
     *             Optional<TipoBebida> tipoBebidaExistente =
     *             tipoBebidaRepository.findById(id);
     *             if (tipoBebidaExistente.isPresent()) {
     *             tipoBebida.setId(id); // Asegura que el ID sea el mismo que el
     *             proporcionado en la URL
     *             TipoBebida tipoBebidaActualizada =
     *             tipoBebidaRepository.save(tipoBebida);
     *             return new ResponseEntity<>(tipoBebidaActualizada,
     *             HttpStatus.OK);
     *             } else {
     *             return new ResponseEntity<>(HttpStatus.NOT_FOUND);
     *             }
     ...
     * 
     *             @DeleteMapping("/{id}")
     *             public ResponseEntity<TipoBebida> borrarTipoBebida(@PathVariable
     *             String id) {
     *             Optional<TipoBebida> tipoBebida =
     *             tipoBebidaRepository.findById(id);
     *             if (tipoBebida.isPresent()) {
